#!/bin/bash
#This script is to send a mail from PC via GUI interface shell script. (Xdialog)
#
...
sed -i "/^/d" $OUTPUT
#-----------Ask for Attachments---------------------------#
Xdialog --backtitle "ANJI BABU KAPAKAYALA" \
...
#-----------Get Confiramation-------------------------#
Xdialog --yes-label "Send" \
--backtitle "ANJI BABU KAPAKAYALA" \
--title "Confirmation" \
--yesno "Press Send button to send a mail  " 8 60 
respose=$?
# make a decsion 
case $respose in
  0) 
